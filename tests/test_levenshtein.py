...
class TestComputeLevenshtein1ToNBytes:
    def test_basic(self):
        result = giga_levenshtein.compute_levenshtein_1_to_n(
        ...
        result = giga_levenshtein.compute_levenshtein_1_to_n(b"kitten", right)
    ...
    def test_empty_right_list(self):
        result = giga_levenshtein.compute_levenshtein_1_to_n(b"hello", [])
        assert result == []

    def test_single_element(self):
        result = giga_levenshtein.compute_levenshtein_1_to_n(b"abc", [b"abc"])
        assert result == [0]
...
class TestComputeLevenshteinMToNBytes:
    def test_basic(self):
        result = giga_levenshtein.compute_levenshtein_m_to_n(
            [b"kitten", b"sitting"],
            [b"kitten", b"sitting"],
        )
    ...
    def test_empty_left_list(self):
        result = giga_levenshtein.compute_levenshtein_m_to_n([], [b"abc", b"def"])
        assert result == []
    ...
    def test_single_pair(self):
        result = giga_levenshtein.compute_levenshtein_m_to_n([b"saturday"], [b"sunday"])
        assert result == [[3]]

    def test_symmetric(self):
        left = [b"abc", b"def", b"ghi"]
        right = [b"xyz", b"abc"]
        result = giga_levenshtein.compute_levenshtein_m_to_n(left, right)
